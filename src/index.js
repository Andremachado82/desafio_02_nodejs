...
function checksCreateTodosUserAvailability(req, res, next) {
  ...
  if (user.pro === true || user.todos.length < 10) {
    ...
    return res.status(403).json({ error: 'User already has the free plan or already has 10 registration of todos' });
  }
...
function checksTodoExists(req, res, next) {
  ...
  if (!validate(id)) {
    return res.status(400).json({ error: 'ID is not uuid valid' })
  }

  const todo = user.todos.find(todo => todo.id === id);

  if (!todo) {
    return res.status(404).json({ error: 'does not exist at todo with this ID' })
  }
...
function findUserById(req, res, next) {
  ...
  if (!user) {
    return res.status(404).json({ error: 'Id does not belong to any users' });
  }
